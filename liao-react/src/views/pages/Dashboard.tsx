                ...
                {articles.map((article, idx) => (
                    ...
                    >
                        <div
                            ...
                            className="bg-white rounded-lg md:rounded-xl overflow-hidden shadow-sm hover:shadow-xl transition-all duration-300 border border-gray-100 h-full flex flex-col cursor-pointer"
                        >
                            <div className="h-20 md:h-48 overflow-hidden relative group">
                                {article.images && article.images.length > 0 ? (
                                    ...
                                    <div className="w-full h-full bg-gradient-to-tr from-gray-200 to-gray-300 flex items-center justify-center text-gray-500 text-[10px] md:text-base">
                                        LIAO
                                    </div>
                                )}
                            ...
                            <div className="p-2 md:p-6 flex-1 flex flex-col">
                                <span className="text-[8px] md:text-xs font-bold text-green-600 uppercase tracking-wider mb-1 md:mb-2">
                                    {article.tags?.[0] || 'Novidade'}
                                </span>
                                <h3 className="text-[10px] md:text-lg font-bold text-gray-900 mb-1 md:mb-2 leading-tight line-clamp-2">
                                    {article.title}
                                </h3>
                                <p className="text-gray-600 text-[8px] md:text-sm line-clamp-2 md:line-clamp-3 mb-2 md:mb-4 flex-1">
                                    {article.description || article.content}
                                </p>
                                <span className="text-[8px] md:text-xs text-gray-400 font-medium mt-auto">
            ...
            {articles.length > 3 && (
                <>
                    <button
                        ...
                        className="hidden md:block absolute top-1/2 left-2 transform -translate-y-1/2 bg-white/80 text-gray-800 p-2 rounded-full shadow hover:bg-white focus:outline-none z-10 transition-all hover:scale-110"
                    >
                        <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 19l-7-7 7-7" /></svg>
                    </button>
                    <button
                        onClick={(e) => { e.stopPropagation(); nextSlide(); }}
                        className="hidden md:block absolute top-1/2 right-2 transform -translate-y-1/2 bg-white/80 text-gray-800 p-2 rounded-full shadow hover:bg-white focus:outline-none z-10 transition-all hover:scale-110"
                    >
    ...
    return (
        <div className="min-h-screen font-sans">
            ...
            <section className="py-20 bg-gray-900 text-white relative overflow-hidden">
                ...
                <div className="absolute bottom-0 left-0 -ml-20 -mb-20 w-96 h-96 bg-green-500 rounded-full opacity-10 blur-3xl"></div>

                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
                    <div className="flex flex-col md:flex-row justify-between items-end mb-12">
                        <div>
                            <span className="text-sm font-bold text-green-400 uppercase tracking-widest">Atualizações</span>
                            <h2 className="text-4xl font-bold mt-2">Últimas Notícias</h2>
                        </div>
                        <Link
                            to="/newsletter"
                            className="mt-4 md:mt-0 text-white border-b-2 border-green-500 pb-1 hover:text-green-400 transition-colors text-sm font-bold uppercase tracking-wide"
                        >
            ...
            <section className="py-24 bg-gradient-to-br from-blue-900 to-black text-white relative">
                ...
                <div className="max-w-4xl mx-auto px-4 text-center relative z-10">
                    ...
                    <Link
                        to="/prosel"
                        className="inline-block px-10 py-5 bg-green-500 text-white rounded-full font-bold text-lg hover:bg-green-600 hover:shadow-2xl hover:-translate-y-1 transition-all shadow-lg shadow-green-900/50"
                    >
                        Quero me Inscrever
                    </Link>
                </div>
