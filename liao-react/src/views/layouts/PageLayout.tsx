...
const PageLayout: React.FC<PageLayoutProps> = ({ 
    ...
    return (
        <div className={`min-h-screen bg-neutral-50 dark:bg-neutral-900 transition-colors duration-200 ${containerClassName}`}>
            {renderHeader || (
                (title || subtitle) && (
                    <div className={`pt-24 pb-12 px-4 transition-colors ${headerClassName}`}>
            ...
            <div className={`max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 ${(title || subtitle || renderHeader) ? '' : 'py-12'}`}>
                {children}
            </div>
        </div>
    );
